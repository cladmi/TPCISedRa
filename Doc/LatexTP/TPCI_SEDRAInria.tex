...
Click on \textbf{\texttt{Apply}}

\subsubsection{Gforge server git hook configuration}
...
After every commit, gforge git server with contact ci.inria.fr and tell it to
build the projects that use your repository. And so trigger your project Build.
...
\begin{description}
  \item[Note] Using Virtualenv Builder will put the code execution in a separated
    ...
    python installation and allow installing packages for this build .
\end{description}
...
\end{itemize}
Tests should fail for python3, at least for the \texttt{print} statement.
...
It's possible to fill your whole hard-drive to get an \texttt{IOError: No space left on device}
...
Like testing what has been printed by your program.

\paragraph{}In this section you will see \texttt{mock} to test the \texttt{main} function
and verify the printed output.

\paragraph{Testing main} In this case testing the \texttt{main} function which
is like calling the \texttt{sphere} script from command line, means:
\begin{itemize}
...
\subsection{Exercice 3bis (optional)}
...
TODO~:
\begin{itemize}
\item Resolve git merge conflits (keep the flags you added, use the new library generation dependency)
\item Generate and visualize the corresponding coverage report
