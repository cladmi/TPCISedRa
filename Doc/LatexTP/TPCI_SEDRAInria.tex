...
\item Click on \texttt{Add post-build action} and select \texttt{Publish Cobertura Coverage Report} from the drop-down menu, your \textbf{\texttt{Cobertura xml report pattern}} is \texttt{**/target/site/cobertura/coverage.xml} (it is the example provided below the field)
...
But a human readable html report and an xml report for \texttt{Jenkins} are also generated.
...
Since version $4.6$, \texttt{gcc} provides this warning \texttt{-Wfloat-equal}. 
...
On \texttt{Jenkins} side, compiler warnings can be checked in the \texttt{Post build Actions} step of your build project. 
...
\item In \texttt{Jenkins}, add the scan for compiler warnings in the \texttt{Post build Action} of your project
