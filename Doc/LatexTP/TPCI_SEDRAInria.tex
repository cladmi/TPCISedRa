...
To perform this C++ exercice, we rely on three main development tools:

\begin{itemize}
\item A \texttt{C++} compiler, for example \texttt{gcc} or \texttt{clang}.
\item A build process manager : \texttt{CMake} 
\item A testing framework : \texttt{cppunit}.
\end{itemize}

On linux, bsd, mac os X systems suitables versions may simply be installed with the package manager.
...
files \texttt{CMakeLists.txt} : one under the src directory (the
main \texttt{CMakeLists.txt}) and one under the tests directory. 

The API provided by this library is here constitued by a single Sphere class. The
Sphere class offers one object constructor with the radius as parameter and a
volume method.
...
\item create the build environment with cmake,
...
arguments : \texttt{cmake . -D<VARIABLE_NAME>=<VALUE>}. The value remains in
...
On this very simple computation, the benchmark difference between Debug and
Release is not dramatic.

With gcc compiler, we can go beyond \texttt{-O3} optimization flag with the
...
To pass a specific argument to the c++ compiler we use the variable
...
generated specification, on can use the argument \texttt{VERBOSE=1} to make,
...
Connect to the INRIA Continuous Integration web portal: https://ci.inria.fr/ \\
Log in and click ``Dashboard''in the top menu \\
You should have been added to project ``TPCISedRa'', click on the ``Jenkins'' button \\
You may be prompted to log into jenkins, use the same login/passwd as for the ci portal
...
From our Jenkins dashboard page (https://ci.inria.fr/tpcisedra/), click ``New Item'' in the menu on the left
Provide a name for this new item (avoid spaces since it is likely to lead to errors) and select ``Freestyle project''
...
\subsubsection{Exercice 1}
Add a new build step : ``Execute shell'', where you inform Jenkins how to
build and test the project. Choose the \texttt{Debug} configuration for this
build, this will be needed for coverage as we will see later.

In the shell working directory (the result of the command \texttt{/bin/pwd} in
this shell) Jenkins has clone your source directory.

Then save the project and verify the configuration with a click on ``Build
...
This is the method which is tested in the unique test of the library, and the test is
...
On a core i7 laptop and with the previous settings for cmake
the test should still be successfull.
...
Since version 4.6 \texttt{gcc} gcc provides this warning \texttt{-Wfloat-equal}: 

With portability in mind, before adding the flag to the compiler command, we
...
This macro is provided in the TP.cmake module under cmake directory.

To load this TP module, one have to set the cmake module path to this directory:
\begin{lstlisting}
...
On Jenkins side, compiler warnings can be checked in the ``Post build
Actions'' step of your build project. 

TODO
\begin{itemize}
\item In the cmake configuration file, the top \texttt{CMakeLists.txt}, include the TP module.
\item With the provided \texttt{CMake} macro, add the compiler warning to the cmake configuration.
\item Verify that the warning is printed during compilation.
\item Add the scan for compiler warnings in the ``Post build Action''
...
\item Modify the appropiate test in \texttt{TestTP.cpp} so small rounding
  differences do not cause errors, cppunit provides a macro for this : 
...
Coverage with gcov needs a compilation with gcc and the
\texttt{-fprofile-arcs} and \texttt{-ftest-coverage} options.  gcov generates
raw output that need a post-processing. We are going to use the lcov
utility for the generation of html pages and the gcovr utility in order to present parsable results to Jenkins.

A \texttt{CMake} function \texttt{add_test_with_coverage} is provided in the
\texttt{TP} module to simplify the whole setup.

This function add coverage support when the \texttt{coverage} target (\texttt{make coverage}) is built .
%$

\subsubsection{Exercice 3}

\begin{itemize}
\item Install coverage for the test TestVolume.
\item With a web browser open the file \texttt{index.html} under \texttt{TestTP__testVolume} directory
\item In ``Post-Build Actions'' add ``Publish Cobertura Report'' with the cobertura xml report pattern set to \texttt{<to be fixed>.xml}
\item Commit
\end{itemize}
