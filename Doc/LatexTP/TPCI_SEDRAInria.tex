...
In the same time, you will use all these tools on Jenkins and use visualization
...
Add a new build step using \emph{Virtualenv Builder}.

Using a \texttt{virtualenv} puts the code execution in a separated python
...
However, let's assume it's successfull and commit your modifications.
\begin{lstlisting}
$ git commit -m "Precompute 4pi/3"
$ git push
\end{lstlisting}
The test should now fail on \texttt{Jenkins}. See the Console output.
...
Commit, push and verify \texttt{Jenkins} result.
...
It allows coverage and test xml results
...
As source files are not based at root directory \texttt{Cobertura} plugin fails
to locate them.
...
Commit your code and push. Manually re-run a build to get Cobertura
plugin graph (it needs more than one build).
You can click on the report to see per-file coverage infos.
...
Add \texttt{lint} and \texttt{pep8} in \texttt{Jenkins} build step.

To get the reports use \textbf{\texttt{Post build Actions}} named
...
Update \texttt{Jenkins} main build step.
...
Add ChuckNorris post build action. It will make your code famous and better.

It displays you Chuck Norris facts and a picture of Chuck adapted to your build
result (seems like the picture is not displayed but I don't know why?)

\subsubsection{Build Passing icon}

There is a plugin for that. It's called \texttt{Embeddable Build Status},
however it requires the project to have at least 'ViewStatus' to allow everyone
to see it. And it's not configured in this project.
...
%$
For this, you can copy 
\begin{lstlisting}
/gitroot/tpcisedra/users/bremond.git/hooks/post-receive
\end{lstlisting}

and simply modify the login.
...
With our library, one can check that the following code is valid but it may not be
...
With the implicit conversion allowed on the Sphere constructor, the pair of
spheres \texttt{p} is constitued of the sphere \texttt{s1} and the Sphere
\texttt{Sphere(2)} which is not the same as the pair of \texttt{s1} and \texttt{s2}.
In the case of a typo, this may certainly lead to bugs.

A static code analysis may help in the discovery of those potentials bugs.

\paragraph{Let's use cppcheck}

On our project, one can simply have a report with the command~:
