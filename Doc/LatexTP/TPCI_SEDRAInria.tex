...
\paragraph{sorry}Explaining mocks and applying is a task itself,
...
Here for mocking them the \texttt{context\_manager} is used \texttt{with} keyword.
It automatically applies and cleans up the patch.

\paragraph{Mocking outputs} Two methods are presented:

The first one is mockng \texttt{sys.stdout} with a StringIO object and get the written string.
Here it is also done using the \texttt{with} keyword.

Another, presented here by configuring the patch in \texttt{setUp} is to use a
mock object verify the \texttt{write} call arguments.

Both method are valid, it's just for presenting alternatives.

\textbf{TODO} Mock the Sphere class just to show how it works and also the decorator.


Some advices I want to give on patch:
...
\item The patching path can depend on how the module is imported \url{http://www.voidspace.org.uk/python/mock/patch.html#where-to-patch}
\end{itemize}

For every other problems, look at the documentation which is quite complete and
on stackoverflow.
