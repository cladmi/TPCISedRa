...
\newcommand{\hmwkClassInstructor}{\url{http://sed.inrialpes.fr}} % Teacher/lecturer
...
%\begin{lstlisting}
%  <properties>
%    <checkstyle.config.location>mygoogle_checks.xml</checkstyle.config.location>
%  </properties>
%\end{lstlisting}
\end{itemize}
...
\item In \texttt{Jenkins}, add the scan for compiler warnings in the \textbf{\texttt{Post build Action}} of your project. In the \textbf{\texttt{Execute shell}} section, comment the line \texttt{make test}. This is necessary because as the test is failing, the parsing won't be done by Jenkins.
\item Commit the code in order to check the parsing done by \texttt{Jenkins}, then uncomment the \texttt{make test} line.
\item Modify the appropriate test in \texttt{TestTP.cpp} so small rounding
  differences do not cause errors.\\
  \texttt{cppnit} provides a macro for this~:\\
...
\\
Remark~: You can see all \texttt{make} targets using~: \texttt{make help}.
%$
...
Let's test if it works with more than one class. This is optional, if you prefer go directly to the Static analysis section.

Thanks to another development branch, we add a new class \texttt{Alphabet} to
our project~:
\begin{lstlisting}
%%% to be verified !!!
$ git merge tpcisedmain/alpha-cxx
\end{lstlisting}
...
\subsubsection{Exercise 5}
\begin{itemize}
\item Install this check on \texttt{Jenkins} (modify the \textbf{\texttt{Build}}section \textbf{\texttt{Execute shell}} field plus add a \textbf{\texttt{Post-build Action}} for publishing \texttt{cppcheck} results). Beware~: change the \texttt{cppcheck} command argument so it generates an \texttt{XML} output with (\texttt{--xml} option and redirect the standard error to a file named
  \texttt{cppcheck-result.xml}.\\
\texttt{Jenkins} can read this file if configured properly if the appropriate \textbf{\texttt{post-build}} action is set.
