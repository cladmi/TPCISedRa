...
In \texttt{Python} it's done using a \texttt{setup.py} script
...
# Run tests and generate \texttt{XML} test results
...
Adding a bash script to do it would work but as we are working with \texttt{Python},
...
\begin{itemize}
\item \texttt{envlist}~: is the list of \texttt{Python} version you want to run on. Here only \texttt{Python}2.7 for the moment.
...
to see the result (\texttt{py32} for \texttt{Python}3.2). Re-run \texttt{tox}.
