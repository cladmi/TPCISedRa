...
This \texttt{CMake} project aims at providing everything needed to build and
test a dynamic shared and versioned ``TP'' \footnote{``TP'' for ``Travaux
  Pratiques'' in french} library. 
...
A tests directory contains an infrastructure based on cppunit that may be
reused for larger project. Inside this infrastructure a test compare the
computed volume with a known value.

A benchmark program named bench.cpp is also built and linked with the
...
in order to see the flags passed to compiler.
...
\subsubsection{Exercice 1}
Add a new build step : ``Execute shell'', where you tell Jenkins how to build
and test the project. Choose the \texttt{Debug} configuration for this build.

The shell working directory (the result of the command \texttt{/bin/pwd} in
this shell) is the directory where Jenkins has clone your source directory.
...
The strict comparison of double should be avoided, we first add a warning
(only if understood by the compiler) in order not to reproduce this kind of
error.

With \texttt{CMake} a correct way to add a compiler flag is:
...
TODO
\begin{itemize}
\item Add the compiler warning to the cmake configuration.
