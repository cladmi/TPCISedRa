...
make test
Running tests...
Test project /home/maurice/src/TPCISedRa/Src/Cxx/build/2
    Start 1: TestVolume
...
Why does the test in \texttt{TestTP.cpp} fail ?
...
\item In some cases, we want to be aware that something has changed, even when
  the change is the tiniest. In that case, the test we already have is just
  what we want.

\item In other cases, we need not worry about such a small difference and
  hence do not want to be bothered by tests complaining.
\end{itemize}

TODO :
[C++] draft doc + cmake cobertura coverage (step4)
\begin{itemize}
\item Find a way to modify the appropiate test in \texttt{TestTP.cpp}
so small rounding differences do not cause errors.
\end{itemize}
