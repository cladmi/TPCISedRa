...
order to present parsable results to Jenkins.
...
\texttt{add_test} function provides coverage support through a new
...
\begin{itemize}
\item Install coverage for the test TestVolume, you need to set the coveraged flags for the build of the library and the test with the add_cxx_compiler_flags and to use the 
\item With a web browser open the file \texttt{index.html} under \texttt{TestTP__testVolume} directory.
\item On the Jenkins side, in ``Post-Build Actions'' add ``Publish Cobertura Report'' with the cobertura xml report pattern set to \texttt{<to be fixed>.xml}
\item Commit
\end{itemize}
